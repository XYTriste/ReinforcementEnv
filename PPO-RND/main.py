...
if __name__ == '__main__':
    ...
    if not config["do_test"]:
        ...
        init_dones = np.zeros(rollout_base_shape, dtype=np.bool_)
        ...
        init_action_efficient = np.ones(rollout_base_shape, dtype=np.bool_)
        ...
        for iteration in tqdm(range(init_iteration + 1, config["total_rollouts_per_env"] + 1)):
            ...
            total_action_efficient = init_action_efficient
            ...
            for t in range(config["rollout_length"]):
                ...
                infos = []
                for worker_id, parent in enumerate(parents):
                    s_, r, d, info, efficient = parent.recv()
                    infos.append(info)

                    total_action_efficient[worker_id, t] = efficient
        ...
        checkpoint = logger.load_weights()
        play = Play(config["env_name"], brain, checkpoint, render_mode=render_mode)
        play.evaluate()
